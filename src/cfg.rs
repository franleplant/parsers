...
impl CFG {
    pub fn new<T: Into<String> + Debug + Ord + Clone>(vn: Set<T>,
                                                      vt: Set<T>,
                                                      prod_vec: GenericProductions<T>,
                                                      s: T)
                                                      -> CFG {
        ...
        let prod_vec: Productions = prod_vec
            .iter()
            .cloned()
            .map(|(nt, der)| (nt.into(), der.iter().cloned().map(|el| el.into()).collect()))
            .collect();


        assert!(vn.is_disjoint(&vt),
                "VN and VT must be disjoint.\nVN: {:?} \nVT: {:?}",
                vn,
                vt);
        ...
        for i in 0..len {
            ...
            assert!(vn.contains(nt),
                    "NonTerminal in production rule does not belong to VN {:?} -> {:?} \n {:?}",
                    nt,
                    derivation,
                    vn);
            for c in derivation {
                assert!(vn.contains(c) || vt.contains(c),
                        "Char in derivation {:?} -> {:?} does not belong to VN or VT {:?}",
                        nt,
                        derivation,
                        c);
            }
